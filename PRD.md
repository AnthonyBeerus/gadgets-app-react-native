# Muse — Powered by Molapo Creative Hub — Product Requirements Document (PRD)
...
* **Molapo Mall Marketplace** — products from Molapo Crossing vendors.
* **Indie Marketplace** — products from independent creators and artisans.

**Current Status**:
* ✅ `malls` table exists (Physical vs Online).
* ✅ `shops` and `products` tables exist.
* ✅ Mall switching logic implemented in `shop-store.ts`.
...
**Current Status**:
* ✅ `events` and `event_venue` tables exist.
* ✅ Events tab exists in navigation.
...
**Current Status**:
* 🚧 Frontend scaffolded (`challenges.tsx`, `ChallengeCard`).
* ❌ **Missing Database Tables**: `challenges`, `challenge_submissions`.
...
### **4.2 Deliveries**
* **Status**: ✅ `delivery_orders` table exists.
