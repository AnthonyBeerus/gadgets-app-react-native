...
export function Rule({ quiet = false }: { quiet?: boolean }) { const t = useDesignTokens(); return <View style={{ height: quiet ? 1 : 2, backgroundColor: quiet ? t.colors.strokeQuiet : t.colors.stroke }} />; }
export function Plate({ children, style, ...props }: ViewProps) { const t = useDesignTokens(); return <View style={[styles.plate, { borderColor: t.colors.stroke, backgroundColor: t.colors.surface }, style]} {...props}>{children}</View>; }
export function OffsetPlane({ children, style, ...props }: ViewProps) { const t = useDesignTokens(); return <View style={[styles.offsetWrap, style]} {...props}><View pointerEvents="none" style={[styles.back, { borderColor: t.colors.stroke, backgroundColor: t.colors.surfaceSunken }]} /><Plate style={styles.front}>{children}</Plate></View>; }
