...
export default function MerchantScanScreen() {
  ...
  if (!permission.granted) return <View style={styles.centered}><Text variant="h2" align="center">Camera access needed</Text><Text variant="body" align="center">Muse scans the buyer's single-use collection code.</Text><Button onPress={requestPermission}>Allow camera</Button></View>;
  ...
  return <View style={styles.container}><Button variant="secondary" style={styles.back} onPress={() => router.back()}>Close</Button><CameraView style={styles.camera} facing="back" onBarcodeScanned={busy ? undefined : scan} barcodeScannerSettings={{ barcodeTypes: ['qr'] }}><View style={styles.overlay}><View style={styles.box} />{busy ? <ActivityIndicator color={colors.payout} /> : <Text variant="h3" align="center" style={styles.copy}>Scan the buyer's collection code</Text>}</View></CameraView></View>;
}
